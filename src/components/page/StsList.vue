    ...
    <div class="table">
        <div class="crumbs">
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item>语音标注列表</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        ...
        <div class="handle-box">
            ...
            <el-select v-model="search_field" placeholder="搜索字段" class="handle-select mr10">
                <el-option key="2" label="用户ID" value="userId"></el-option>
                <el-option key="3" label="文档ID" value="docId"></el-option>
            </el-select>
        ...
        <el-table :data="docs" border style="width: 100%" ref="singleTable" @current-change="">
            ...
            <el-table-column label="操作" width="140">
                <template scope="scope">
                    <el-button size="small" v-if="scope.row.isWavOnlyUploaded"
                               @click="onMark(scope.$index, scope.row)">去标注</el-button>
...
<script scoped>
    ...
    export default {
        ...
        created(){
            ...
            this.isAdmin = query.isAdmin || false;
        ...
        methods: {
            ...
            getData() {
                ...
                self.$axios.get(self.url, { params }).then((res) => {
                    ...
                    for (let i = 0; i< docs.length; i++) {
                        ...
                        docs[i].isWavOnlyUploaded = docs[i].wavCount > 0 && !docs[i].isWavMarked;
                    }
                    self.docs = docs;
            ...
            onEdit(index, row) {
                ...
                this.$router.push({ path: `/sts_new?id=${row._id}&version=${version}&taskId=${encodeURIComponent(row.taskId)}&search_field=${encodeURIComponent(this.search_field)}&search_key=${encodeURIComponent(this.search_key)}`});
            },
            onMark(index, row) {
                this.$router.push({ path: `/sts_new?id=${row._id}&isMarkNew=1&taskId=${encodeURIComponent(row.taskId)}&search_field=${encodeURIComponent(this.search_field)}&search_key=${encodeURIComponent(this.search_key)}`});
            },
            showComfirm() {
