    ...
    <div class="table">
        <div class="crumbs">
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item>语音标注导出管理</el-breadcrumb-item>
            </el-breadcrumb>
        </div>

        <div class="handle-box">
            <el-input v-model="date" placeholder="输入日期(如20180126)" class="handle-input mr10"></el-input>
            <el-button type="primary" icon="search" @click="onExport">导出</el-button>
        </div>
        <el-table :data="docs" border style="width: 100%" ref="singleTable" @current-change="">
            ...
            <el-table-column prop="date" label="对应日期"></el-table-column>
            <el-table-column prop="totalDuration" label="原始时长(毫秒)"></el-table-column>
            <el-table-column prop="checkedDuration" label="有效时长(毫秒)"></el-table-column>
            <el-table-column prop="total" label="目录数" width="100"></el-table-column>
            <el-table-column prop="dir" label="存放路径" width="150"></el-table-column>
...
<script scoped>
    ...
    export default {
        ...
        methods: {
            ...
            onExport(index, row) {
                this.$axios.post(this.url, {date: this.date}).then((res) => {
                    setTimeout(() => {
                        this.getData();
                    }, 1000);
                });
