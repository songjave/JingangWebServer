...
(function(window, undefined) {
   ...
   */
  tracking.initUserMedia_ = function(element, opt_options, cb) {
    window.navigator.getUserMedia({
      ...
      audio: !!(opt_options && opt_options.audio)
        ...
        return cb({
            errCode: -6,
            errMsg: 'Cannot capture user camera.'
        })
      }
    );
  };
   ...
   * @param {object} opt_options Optional configuration to the tracker.
   */
  tracking.track = function(element, tracker, opt_options, cb) {
    ...
    switch (element.nodeName.toLowerCase()) {
      case 'canvas':
        return cb(this.trackCanvas_(element, tracker, opt_options));
      case 'img':
        return cb(this.trackImg_(element, tracker, opt_options));
      case 'video':
        if (opt_options) {
          if (opt_options.camera) {
            this.initUserMedia_(element, opt_options, (err) => {
                return cb(err);
            });
          }
        }
        return cb(this.trackVideo_(element, tracker, opt_options));
      default:
